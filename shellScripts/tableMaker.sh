...
for zGroup in $zeniths; do 
	...
	while (( noiseIndex < noiseNum )); do 
	    oGroupName=${oGroup//./}
	    simFileList=$workDir/config/simFileList_${array}_ATM${atm}_Z${zGroup//,/-}_${oGroupName//,/-}wobb_${noiseIndex//,/-}noise.txt 

	    oGroupNoDot=${oGroup//./}
	    tableFileBase=${mode}_${model}_ATM${atm}_${simulation}_vegas254_7sam_${oGroupNoDot//,/-}wobb_Z${zGroup//,/-}_std_${DistanceUpper//./p}_${noiseIndex}noise #modify zeniths, offsets
...
trap "rm $smallTableFile; mv $logFile $workDir/rejected/; exit 130" 1 2 3 4 5 6 
...
case $mode in 
    dt)
	buildCmd="buildDispTree $dtWidth $dtLength -DTM_Azimuth=${azimuths// /,} -DTM_Zenith=${zeniths// /,} -DTM_Noise=${noises} $workDir/processed/tables/dt_${model}_ATM${atm}_${simulation}_vegas254_7sam_Alloff_LZA_std_${DistanceUpper//./p}_AllNoise.root" ;;
    lt) 
	buildCmd="buildLTTree -TelID=0,1,2,3 -Azimuth=${azimuths// /,} -Zenith=${zeniths// /,} -AbsoluteOffset=${offsets// /,} -Noise=${noises} $workDir/processed/tables/lt_${model}_ATM${atm}_${simulation}_vegas254_7sam_Alloff_LZA_std_${DistanceUpper//./p}_AllNoise.root" ;;
    ea) 
	buildCmd="buildEATree " ;; 
esac # build commands based on mode 			
