...
zenith=LZA # should mean covers all zenith angles 
...
\t\t "
    echo -e "\tenv_Crab.sh will be used as default!"
...
\t\t make sure you use quotes if the option has spaces' 
    echo -e "\t--submit | -q \tsubmit jobs to sbatch"
...
##### NOTE: no variables used here should share names with environment files, otherwise they will be overwritten! ### 
args=`getopt -o l1:2:4:5:c:hB:s:qr:e:p:kdm:n:o:ibL -l run:,submit,queue:,env:,box_cuts:,c4:,c5:,xl:,x1:,x2:,x3:,x4:,d1:,d2:,d4:,d5:,timecuts:,mem:,disp:,atm:,deny:,BDT:,lt:,reprocess,no-docker -n 'process_script.sh' -- "$@"`
...
for i; do  
	...
	--box_cuts|-c) 
	    box_cuts=$2 ; shift 2 ;;
	-r|--run) 
	    runMode="$2" # executable to run on script file, e.g. bash or cat, sbatch 
	    shift 2 ;;
	--submit|q) # equivalent to --run sbatch  
	    runMode=sbatch
	    partition=shared ; shift ;;
	--queue) runMode=sbatch
	    partition=${2} ; shift 2 ;; 
	-m|--mem) mem="$2" ; shift 2 ;; 
	-N|--no-docker) # turns off docker - this switch should be processed in environment file! 
	...
	--atm) # use one atmosphere for every run 
	    ATM=$2 ; shift 2 ;; 
	-e|--env) environment="$2" # has problem resetting box_cuts if it comes after, should load first 
	    shift 2 ;;
	...
	for n in $laser1 $laser2 $laser3 $laser4; do
		...
		if [ "$runLaser" == true ]; then 
			...
			if [ "$runMode" != "print" ]; then
			    ...
			    [ "$runMode" == "sbatch" ] && test -f $logFileLaser && mv $logFileLaser $laserBackup/
			    # hsi command can be executed in shell, but not from docker image 
			    getDataFile $laserData || exit 1  
...
EOF
			    completion=$?
			    [ "$runMode" == "sbatch" ] && test "$completion" -eq 0 && touch $queueFileLaser
			    nJobs=$((nJobs+1))
...
for sig in $signals; do 
    trap "echo -e \"TRAP! Signal: \$sig\"; failed $sig" \$sig
done 
...
EOF
	...
	if (( $numTels < 1 )); then 
	    echo -e "\e[0;31mNo telescopes found for ${runNum}, skipping!\e[0m"
	    continue
	elif (( $numTels < 3 )); then
	...
	if [ "$runStage1" == true -o "$runStage2" == true ]; then
		...
		get_data="getDataFile $dataFile || exit 1 "
...
EOF
	...
	if [ "$completion" -eq 0 ]; then
	    ...
	    nJobs=$((nJobs+1))  
	    #echo "VEGAS job \$PBS_JOBID started on:  " ` hostname -s` " at: " ` date ` >> $laserLog/sbatchLog.txt
	else
