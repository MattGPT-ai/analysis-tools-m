...
# sets the impact distance cut by default, but will not overwrite it if you set it in environment 
test -z "$ImpactDistanceUpper" && ImpactDistanceUpper=300 # new cut to improve sensitivity 
...
# right now these are set in setCuts, but this function also works 
setTableNames() { # zen offset 

    test $MaxHeightLower != -100 && MaxHeightLabel="_MH${MaxHeightLower//./p}" || MaxHeightLabel="" 
    
# to compact zeniths:
    zens_hyph=${zeniths//[ ,]/-}
    zens_hyph="$(echo $zens_hyph)"
    zenrange="${zens_hyph%%-*}-${zens_hyph##*-}"
    zenlabel=Z${zenrange}

    offlabel=${offsets//./} # don't want .s in filenames
    offlabel="$(echo $offlabel)" # strip trailing and leading whitespace 
    offlabel="${offlabel//[, ]/-}off"
    
# set table names 
    ltBase=lt_${model}_${array}_ATM${atm}_${simulation}_${ltVegas}_7samples_${offlabel}_Z${zenrange}_std_d${DistanceUpper//./p}${lt_ext}
    finishedLT=$ltDir/${ltBase}.root
    ltAuto=$ltDir/$(hget ${array}${atm})
#ltAuto=$ltDir/lt_Oct2012_${array}_ATM${atm}_GrISUDet_vegas250rc5_7samples_050wobb_Z${zenrange}_std_d1p43.root
    eaBase=ea_${model}_${array}_ATM${atm}_${simulation}_${ltVegas}_7samples_${offlabel}_Z${zenrange}_std_d${DistanceUpper/./p}_s${SizeLower/0\//}_MSW${MeanScaledWidthUpper/./p}_MSL${MeanScaledLengthUpper/./p}${MaxHeightLabel}_ThetaSq${ThetaSquareUpper/./p}_DistCut${ImpactDistanceUpper}m${ea_ext}
    finishedEA=$eaDir/${eaBase}.root
    
    #archiveLT="$(hget ${array}${atm})"
...
setSimNames() { # zen offset noise stage 
    ...
    simFileBase=Oct2012_${array}_ATM${atm}_${ltVegas}_7samples_${1}deg_${2//./}wobb${sim_ext}_${3}noise
    #simFile=$simDir/${simFileBase}.stage${stage}.root
    #simExt may be set in options in sim_script.sh
