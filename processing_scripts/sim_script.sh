...
# instructions for creating batch job script 
mem=4gb # it's better if this could be 2gb, but some jobs fail - investigate! 
...
args=`getopt -o 4:5:bd:z:o:n:c:h:w:BD:e:x:L: -l box_cuts:,c4:,c5:,x4:,x5:,BDT:,disp:,lt:,cutTel:,override,offsets:,arrays:,atms:,noises:,zeniths:,sim_ext: -n sim_script.sh -- "$@"`
eval set -- $args
for i; do 
    case "$i" in
	-4) runStage4=true 
	...
	--BDT) # actually applies BDTs, requires weights file
	    ...
	    test -d $2 || ( echoErr "Weights directory does not exist!! "; exit 1 ) 
	    shift 2 ;; 
...
checkForDirs $runMode $processDir/$stage4subDir $processDir/$stage5subDir $logDir/$stage4subDir $logDir/$stage5subDir $queueDir $backupDir $logDir/batch_out $completeDir $failDir

nJobs=(0) 

offsets="${offsets//,/ }" # this makes converts list from group format to individual offsets 
...
for array in $arrays; do
	...
	for zen in ${zeniths//,/ }; do 
		...
		for noise in $noises; do
		    ...
		    queueFile_4=$queueDir/${stage4subDir}_${simFileBase}.stage4
		    rootName_5=$processDir/${stage5subDir}/${simFileBase}.stage5.root
		    queueFile_5=$queueDir/${stage5subDir}_${simFileBase}.stage5
		    ...
		    # submit jobs 
		    logFile_batch=$logDir/batch_out/${simFileBase}.txt
		    submitHeader=$(createBatchHeader -N ${stages}_${simFileBase}_${stage4subDir}_${stage5subDir} -o $logFile_batch -t $timeTotal -M $mem -p $priority)

		    if [ "$runMode" != print ] && [ "$runBool" == true ]; then 
			
			[ "$runMode" == bash ] && redirection="| tee $logFile_batch" || redirection=""
			$runMode <<EOF #| tee $logFile_batch
$submitHeader
